...
@MainActor
final class DeletionWindow: NSWindowController, NSWindowDelegate {
    private static var operations: [DeletionWindow] = []
    ...
    static func start(sources: [URL], completion: @escaping @MainActor () -> Void) {
        ...
        let controller = DeletionWindow(sources: sources)
        ...
        Task {
            do {
                let report = try await EntryOperations.moveToTrash(sources, journalDirectory: journals)
    ...
    private init(sources: [URL]) {
        ...
        window.title = L("휴지통 이동", "Move to Trash")
        ...
        label.stringValue = L("휴지통으로 이동 중…", "Moving to Trash…")
    ...
    private func finish(_ report: TrashOperationResult) {
        running = false
        ...
        label.stringValue = L("작업 완료", "Finished") + " · " +
            L("완료 \(completed)/", "Completed \(completed)/") + "\(report.items.count) · " +
            L("실패 \(failed)", "Failed \(failed)")
        details.string = report.items.map { item in
            ...
            return "[" + status + "] " + item.source.path +
                (item.trashed.map { "\n  " + L("휴지통 위치: ", "Trash URL: ") + $0.path } ?? "") +
                (item.message.map { "\n  " + $0 } ?? "")
        }.joined(separator: "\n\n")
    ...
    private func fail(_ error: Error) {
        running = false
        label.stringValue = L("휴지통 이동 실패", "Move to trash failed")
