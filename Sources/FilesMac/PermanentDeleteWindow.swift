...
@MainActor
final class PermanentDeleteWindow: NSWindowController, NSWindowDelegate {
    private static var operations: [PermanentDeleteWindow] = []
    ...
    static func start(sources: [URL], completion: @escaping @MainActor () -> Void) {
        guard !isRunning else { return }
        let controller = PermanentDeleteWindow(sources: sources)
        ...
        Task {
            do {
                let report = try await EntryOperations.permanentlyDelete(sources, journalDirectory: journals)
    ...
    private init(sources: [URL]) {
        ...
        label.stringValue = L("항목을 영구 삭제 중입니다…", "Permanently deleting items…")
        ...
        for view in [label, scroll] { view.widthAnchor.constraint(equalTo: stack.widthAnchor).isActive = true }
    }

    required init?(coder: NSCoder) { fatalError("init(coder:) is not supported") }

    private func finish(_ report: PermanentDeleteOperationResult) {
        running = false
        let completed = report.items.filter { $0.state == "completed" }.count
        let failed = report.items.filter { $0.state == "failed" || $0.state == "notFound" }.count
        label.stringValue = L("작업 완료", "Finished") + " · " +
            L("영구 삭제 완료 ", "Permanently deleted ") + "\(completed)/\(report.items.count)" +
            L("개 항목", " items")
        if failed > 0 {
        ...
        details.string = report.items.map { item in
            ...
            case "completed":
                status = L("완료", "Done")
            case "notFound":
                status = L("삭제할 항목 없음", "Source missing")
            case "failed":
                status = L("실패", "Failed")
            default:
                status = L("미처리", "Not attempted")
            }
            return "[\(status)] " + item.source.path + (item.message.map { "\n  " + $0 } ?? "")
        }.joined(separator: "\n\n")
    ...
    private func fail(_ error: Error) {
        running = false
        label.stringValue = L("영구 삭제 실패", "Permanent delete failed")
