...
| 항목 ID | M1에서는 경로 기반 directory-entry ID | hard link 중복 표시 방지 테스트 통과; 외부 rename 선택 유지 미지원 |
| 파일 종류 | 기본 메타데이터와 localizedTypeDescription을 분리; 현재 종류는 폴더/확장자 표시 | 부가 API 실패가 폴더 판별을 훼손하지 않도록 수정 |
| 변경 감시 | 방문 폴더의 mtime을 2초 간격 확인하는 초기 구현 | FSEvents/세밀한 metadata 변경 감시는 후속 작업 |
